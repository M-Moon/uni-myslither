...
class MySlitherModel {
    ...
    final Map<Integer, Food> poisonFoods = new LinkedHashMap<>();
    ...
    void addPoisonFood(int x, int y, double size, boolean fastSpawn) {
        synchronized (view.modelLock) {
            poisonFoods.put(y * gameRadius * 3 + x, new PoisonFood(x, y, size, fastSpawn));
        }
    }
    ...
    void removePoisonFood(int x, int y) {
        synchronized (view.modelLock) {
            poisonFoods.remove(y * gameRadius * 3 + x);
        }
    }
